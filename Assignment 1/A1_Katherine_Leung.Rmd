...
This report will analyse bulk RNA sequencing data from the GEO database, accession number: GSE213001, which looks at the lung tissue of people affected and unaffected by idiopathic pulmonary fibrosis (IPF). This data is associated with an article by Jaffar et al. published in 2022 *****citation. 

Samples were collected from 20 diseased patients and 14 non-diseased controls. 98 samples were collected from diseased tissues and 41 were collected from healthy tissues, for a total of 139 samples. 

# Obtaining and assessing the data
The data was obtained using the GEOquery Bioconductor package ****citation. 
...
table(pheno$characteristics_ch1.7)
...
kable(table(table(patients)), 
      ...
      caption = "Distribution of Samples per Patient")

```
**Table 1. Distribution of samples per patient** 
...
Using Ensembl [CITATION], Ensembl IDs found in the expression data were mapped to HGNC symbols, which are more readable and useful for downstream processing. 
...
expr_annot <- merge(id_conversion, expr_matrix, 
                    by.x = 1, by.y = 0, all.y = TRUE)

# View results
kable(head(expr_annot[, 1:5]), caption = "Preview of expression data with mapped HGNC symbols")
...
```
**Table 2. Preview of expression data mapped to HGNC symbols**
...
Most samples appear to have a similar mean CPM. With so many samples, it is difficult to visualize the entirety of the data. 
...
As expected, there is somewhat of a peak near zero counts as many genes are expressed in low quantity. 

## Filter out genes with low expression
EdgeR was again used to filter out genes with low expression levels (counts < 3). 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
Again, a density plot of CPM after filtering: 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
It is important to use a design matrix as it allows filtering to be done in a way that considers our conditions. Without one, genes that are differentially expressed between conditions, in this case diseased vs non diseased patients, could be filtered out if they have low counts in one of the two conditions.  

Information on whether the sample is from a diseased on healthy person is found in the metadata. 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
Compared to the initial plot, we can see that the number of genes with low counts has decreased. We should expect a similar result after normalization.
...
```
**Figure 8. Box plot of Trimmed Mean of M-values (TMM)-normalized gene expression** for all 139 samples. The default minimum count is used. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
plot_density(norm_cpm, main = "TMM-Normalized Gene Expression")
```
**Figure 9. Box plot of Trimmed Mean of M-values (TMM)-normalized gene expression** for all 139 samples. The default minimum count is used. 

Compared to the original data (Figure 3), we can see that genes with low counts have been removed and the curve is overall more clean. 

## MDS Plot 
Next, we represented the data in an a multidimensional scaling (MDS) plot to visualize the similarities and differences of this high dimensional data.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# MDS plot
...
```
**Figure 10. Multidimensional scaling (MDS) plot of TMM normalized data** Red circles represent healthy donor samples, and black triangles represent diseased donor samples. 

By inspection, there is not a clear separation between diseased and normal tissues. 
...
plotBCV(edge, main= "Biological Coefficient of Variance Plot")
...
Some points have very high dispersion (maximum of 7.55), which could be due to each of the 139 samples being treated independently. Of the three most widely used differential expression analysis tools (limma, EdgeR and DESeq2), which perform similarly [@liu_three_2021], limma [CITATION] has the advantage of the duplicateCorrelation function, which models within-patient correlation (takes into account multiple samples coming from one individual). This is important as many patients contributed multiple samples to this data set, and treating them as indepent could inflate variance measurements. Voom was used to transform data from EdgeR so that it can be used by limma [CITATION]. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Design column names are not syntactically valid, change: 
colnames(design) <- c("Disease", "Normal")                                                                                                                 
# Make contrast: disease vs normal
contr <- makeContrasts(
...
A threshold of 0.05 was chosen for both p-value and FDR/multiple hypothesis correction as this is the standard cutoff used. After differential expression analysis, we can see that 8192 genes pass the p < 0.05 threshold. 7182 pass FDR correction (at most 5% of significant genes are expected to be false positives).
Next, we can look at the top genes which are the most differentially expressed. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
results <- edgeR::topTags(qlf, n = Inf)$table %>%
  rownames_to_column("HGNC symbol")

kable(head(results), caption = "Top differentially expressed genes", digits = 50)
```
**Table 3. Preview of top differentially expressed genes**
...
kable(head(top_hits[, c("gene", "logFC", "FDR")]), digits = 50)
...
disease_cols <- setNames(c("maroon", "palegreen4"), levels(disease))
...
```
**Figure X. Heatmap of top 20 differentially expressed genes** 
...
I read the paper and appreciated its brevity and straightforwardness. I hadn't heard of Idiopathic pulmonary fibrosis (IPF) before, and the topic interested me. 
...
There were none. See end of  [Obtaining and assessing the data](#obtaining-and-assessing-the-data)
